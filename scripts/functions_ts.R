...
#Topic plot
topic_plot <- function(df) {
  df %>% 
    select(-stories) %>% ungroup() %>% 
    pivot_wider(names_from=prepost, values_from=pct_stories) %>% 
    replace_na(list(`1`=0)) %>% 
    mutate(pct_change=`1`/`0`-1) %>% 
    ggplot(aes(x=`0`, y=topic, xend=`1`, yend=topic)) + geom_segment(arrow=arrow(length = unit(0.2, "cm"))) + 
    labs(x="% of stories", y="Topic", title="Composition pre/post layoffs")
}